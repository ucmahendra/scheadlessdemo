...
try {
    ...
    & $mkcert -install
    & $mkcert -key-file key.pem -cert-file cert.pem "*.vmldemo.localhost"
...
# CM_HOST
Set-EnvFileVariable "CM_HOST" -Value "cm.vmldemo.localhost"

# ID_HOST
Set-EnvFileVariable "ID_HOST" -Value "id.vmldemo.localhost"

Set-EnvFileVariable "SITECORE_GRAPHQL_UPLOADMEDIAOPTIONS_ENCRYPTIONKEY" -Value (Get-SitecoreRandomString 16 -DisallowSpecial)

# RENDERING_HOST
Set-EnvFileVariable "RENDERING_HOST" -Value "www.vmldemo.localhost"
...
try {
    ...
    Write-Host
    Write-Host "vmldemo Developers:  Please add this certificate to your Trusted Root Certification Authorities." -ForegroundColor Red
    Write-Host
